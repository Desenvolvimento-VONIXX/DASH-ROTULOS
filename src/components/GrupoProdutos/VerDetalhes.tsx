...
const ModalVerDetalhes: React.FC<Props> = ({
  open,
  onClose,
  idClick,
  ...
  const [openReabrirRotulo, setOpenReabrirRotulo] =
    React.useState<boolean>(false);
  const [openVerChecklist, setOpenVerChecklist] =
    React.useState<boolean>(false);
  ...
  const {
    ...
    isError,
    isLoading,
    refetch: refetchLinhaTempo,
  } = useGetLinhaTempo(idClick);
                ...
                <div key={rotulo.ID_ROTULO}>
                  <p className="font-semibold mb-1">
                    ...
                    <span className="font-light">
                      {rotulo.CODPROD} - {rotulo.DESCRPROD.trim()}
                    </span>
                  </p>
                  <p className="font-semibold mb-1">
                  ...
                  <p className="font-semibold mb-1">
                    Data de Criação:{" "}
                    <span className="font-light">{rotulo.DATA}</span>
                  </p>

                  <div className="flex gap-2 mb-5 mt-2">
                    {rotulo.STATUS === "PENDENTE" ||
                    rotulo.STATUS === "REPROVADA" ? (
                    ...
                    {rotulo.ABERTOPDI === "S" && <Formula idClick={idClick} />}
                  </div>
                </div>
              ) : null}
            </div>
