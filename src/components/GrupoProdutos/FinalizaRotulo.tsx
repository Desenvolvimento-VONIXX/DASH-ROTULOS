...
const FinalizarRotulo: React.FC<Props> = ({
  open,
  onClose,
  idClick,
  refetchRotulo,
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      if (response.status === "1") {
        ...
        refetchLinhaTempo();
        form.reset();
        toast.success("Rótulo/Cartonado Finalizado!", {
          duration: 4000,
  ...
  const anexaArquivos = async (files: File[], id: number) => {
    ...
    for (const file of files) {
      ...
      try {
        ...
        await JX.novoSalvar({ ANEXO: sessionKeyList }, "AD_ROTULOSATIVIDADE", {
          ID_ROTULO: idRotulo,
          ID_ATIVIDADE: id,
        });
      } catch (error) {
  ...
  return (
    <Sheet open={open} onOpenChange={onClose}>
      <SheetContent>
        <SheetHeader>
          <SheetTitle>Finalizar Rótulo/Cartonado</SheetTitle>
          <SheetDescription>
            Caso o rótulo/cartonado não precise ser avaliado pelo SGI, você pode
            finalizá-lo diretamente. Preencha os campos abaixo para concluir o
            processo.
          </SheetDescription>
        </SheetHeader>
        <FormProvider {...form}>
                    ...
                    <FormItem className="flex-1">
                      ...
                      <FormControl>
                        <Textarea
                          placeholder=""
                          {...field}
                          className="h-20 border-2 border-gray-300 dark:border-gray-500 rounded-lg"
                        />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
