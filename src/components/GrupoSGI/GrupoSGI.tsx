...
import ModalVerDetalhesSGI from "./VerDetalhesSGI";
import { useRotulosSGI } from "@/hook/GrupoSGI/getRotulos";
...
const GrupoSGI: React.FC<Props> = () => {
  ...
  const { data: rotulos, refetch } = useRotulosSGI();
  ...
  const uniqueStatuses: string[] = rotulos
    ? Array.from(
        new Set(rotulos.map((rotulo: Rotulo) => rotulo.STATUS2.trim()))
      )
    : [];

  const filteredRotulos = (rotulos || []).filter((rotulo: Rotulo) => {
    const dataEnvio = rotulo.DATA_ENVIO
      ? (rotulo.DATA_ENVIO, "dd/MM/yyyy", new Date())
      : null;
    const dataFinalizacao = rotulo.DATA_FINALIZACAO
      ? (rotulo.DATA_FINALIZACAO, "dd/MM/yyyy", new Date())
      : null;

    const filtroDataEnvio =
      dateInicial?.from && dataEnvio
        ? dataEnvio >= dateInicial.from &&
          (!dateInicial.to || dataEnvio <= dateInicial.to)
        : !dateInicial?.from;

    const filtroDataFinalizacao =
      dateFinal?.from && dataFinalizacao
        ? dataFinalizacao >= dateFinal.from &&
          (!dateFinal.to || dataFinalizacao <= dateFinal.to)
        : !dateFinal?.from;

    return (
      (statusFilter && statusFilter !== "all"
        ? rotulo.STATUS2 === statusFilter
        : true) &&
